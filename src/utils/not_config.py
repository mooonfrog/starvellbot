...
from src.utils.console import BOLD, CYAN, DIM, RESET, WHITE, YELLOW, fail_mark, info_mark, success_mark
BANNER = '\n\n   ┌─┐┌┬┐┌─┐┬─┐┬  ┬┌─┐┬  ┬  ┌┐ ┌─┐┌┬┐\n   └─┐ │ ├─┤├┬┘└┐┌┘├┤ │  │  ├┴┐│ │ │\n   └─┘ ┴ ┴ ┴┴└─ └┘ └─┘┴─┘┴─┘└─┘└─┘ ┴\n\n   ʕ•ᴥ•ʔ  sᴛᴀʀᴠᴇʟʟʙᴏᴛ  -  by @yusxe\n   plugins: t.me/excplugins\n\n   github: github.com/mooonfrog/starvellbot\n\n'
KAOMOJI_HELLO = 'ʕ•ᴥ•ʔ'
KAOMOJI_HAPPY = '(*^▽^*)'
KAOMOJI_LOVE = '(♥ᴗ♥)'
KAOMOJI_TADA = '※\\(^o^)/※'
...
def _ask(label: str, kao: str=KAOMOJI_HAPPY, default: Optional[str]=None, optional: bool=False) -> str:
...
def run_setup_wizard(directory: Path) -> Config:
    print(f'{info_mark()} Конфиги в {directory} не настроены. Прогоним опросник, это быстро {KAOMOJI_HELLO}\n')
    ...
    cfg.telegram_bot_token = _ask('Введите Telegram Bot Token (от @BotFather)', kao=KAOMOJI_TADA)
    while True:
...
def ensure_config(directory: Path) -> Config:
    cfg = load_config(directory)
    needs_wizard = not cfg.session_cookie or not cfg.telegram_bot_token or (not cfg.telegram_password_hash) or (not cfg.telegram_password_hash.get('hash'))
    if needs_wizard:
