...
def _verify_password(password: str, stored: dict) -> bool:
    if not stored or not stored.get('hash'):
        return False
    try:
        check = hashlib.scrypt(password.encode('utf-8'), salt=bytes.fromhex(stored['salt']), n=2 ** 14, r=8, p=1, dklen=32)
...
class Config:
    ...
    disabled_reviews: list[str] = field(default_factory=list)
    thanks_after_complete_enabled: bool = False
    thanks_text: str = ''
    greeting_enabled: bool = False
    greeting_text: str = ''
    greeted_chat_ids: list[str] = field(default_factory=list)
    send_message_rate_per_minute: int = 20
    autobump_enabled: bool = False
    autobump_interval: int = 15
    autobump_list: list[dict] = field(default_factory=list)
    ...
    def load(cls, directory: Path=CONFIG_DIR) -> 'Config':
        ...
        cfg = cls()
        cfg._dir = directory
        files_exist = any(((directory / p.name).exists() for p in (FILE_MAIN, FILE_TG, FILE_SETTINGS)))
        ...
        if not files_exist:
            cfg.save()
            log.warning('конфиги созданы в %s. заполни session_cookie, telegram_bot_token и пароль', directory)
        return cfg
    ...
    def verify_password(self, user_id: int, password: str) -> tuple[bool, Optional[str]]:
        with self._lock:
            ...
            if attempts_left <= 0:
                ...
                self.telegram_password_attempts[str(user_id)] = {'count': 0, 'ban_until': ban_until}
                self.save()
                return (False, f'Превышен лимит попыток. Доступ заморожен на {self.telegram_password_ban_minutes} мин.')
    ...
    def get_starvell_command(self, token: str) -> Optional[str]:
        return self.commands.get(token.lower())

    def set_starvell_command(self, token: str, response: str) -> None:
        with self._lock:
            self.commands[token.lower()] = response
            self.save()

    def remove_starvell_command(self, token: str) -> bool:
        with self._lock:
            if token.lower() not in self.commands:
                return False
            del self.commands[token.lower()]
            self.save()
            return True
    ...
    def review_reply_for(self, rating: int) -> Optional[str]:
        if str(rating) in self.disabled_reviews:
            return None
        return self.review_replies.get(str(rating))

    def toggle_review_rating(self, rating: int) -> bool:
        r_str = str(rating)
        with self._lock:
            if r_str in self.disabled_reviews:
                self.disabled_reviews.remove(r_str)
                disabled = False
            else:
                self.disabled_reviews.append(r_str)
                disabled = True
            self.save()
            return not disabled
