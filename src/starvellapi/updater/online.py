...
class OnlineKeeper:
    ...
    async def _connect_and_listen(self) -> None:
        headers = {
            "User-Agent": self._account._user_agent,
            "Origin": "https://starvell.com",
            "Cookie": (
                f"session={self._account._session_cookie}; "
                "starvell.theme=dark; "
    ...
    async def _read_open(self, ws) -> tuple[int, int]:
        ...
        return (
            int(payload.get("pingInterval", _DEFAULT_PING_INTERVAL_MS)),
            int(payload.get("pingTimeout", _DEFAULT_PING_TIMEOUT_MS)),
        )
